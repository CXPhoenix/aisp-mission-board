...
# TODO: 確認新的 `get_mission_id` 是否有用。
def get_mission_id(mission_obj):
    """Helper function to safely extract mission ID from Link or Mission object"""
    # Check if it's a Link object (has fetch method)
    if hasattr(mission_obj, "fetch") and hasattr(mission_obj, "ref"):
        # It's a Link object, access the referenced object's ID
        return str(mission_obj.ref.id) if hasattr(mission_obj.ref, "id") else None
    # Check if it's a direct Mission object
    elif hasattr(mission_obj, "id"):
        # It's a Mission object, get ID directly
        return str(mission_obj.id)
    else:
        # Unknown object type
        return None

# deprecated
# def has_user_completed_mission(user: User, mission_id):
...
async def has_user_completed_mission(user: User, mission_id: str | PydanticObjectId):
    """Check if user has already completed a specific mission"""
    mission_submitted_filter = (
        MissionSubmitted.session == app_conf.mission,
        MissionSubmitted.user_id == str(user.id),
        MissionSubmitted.mission_id == str(mission_id),
        MissionSubmitted.review_status == MissionReviewState.APPROVED,
    )
    mission_submitted_list = await MissionSubmitted.find(*mission_submitted_filter).to_list()
    return len(mission_submitted_list) > 0
...
async def mission_index_page(
    ...
    # 獲取用戶當前進行中的任務ID列表
    ongoing_mission_ids = [
        get_mission_id(mission) for mission in current_user.ongoing_missions
    ]
    ...
    for mission in available_missions:
        ...
        has_completed = (await has_user_completed_mission(current_user, mission_id))

        # 檢查該任務是否正在 Review 中
        existing_review_mission = await MissionSubmitted.find_one(
            MissionSubmitted.mission_id == mission_id,
            MissionSubmitted.user_id == str(current_user.id),
            MissionSubmitted.session == app_conf.mission,
        ...
        can_accept = (
            not is_accepted
            and not (
                not mission.is_repetitive and has_completed
            )  # 不可重複且已完成的任務不能再接
            and len(current_user.ongoing_missions) < current_user.max_missions
            and current_user.token >= mission.invest
            and (existing_review_mission is None)
        )
...
async def mission_detail_page(
    ...
    can_accept = (
        ...
        and len(current_user.ongoing_missions) < current_user.max_missions
        and current_user.token >= mission.invest
        and current_user.level >= mission.accepted_min_level
        and mission.expired_time > get_now()
        and (existing_review_mission is None)
    )
    ...
    try:
        ...
        if not mission or mission.session != app_conf.mission:
            ...
            return RedirectResponse(
                url=request.url_for("mission_index_page"), status_code=302
            )
        ...
        if current_user.level < mission.accepted_min_level:
            request.session["error"] = (
                f"等級不足，需要等級 {mission.accepted_min_level}"
            )
            return RedirectResponse(
                url=request.url_for("mission_index_page"), status_code=302
            )
        ...
        # 檢查不可重複任務是否已完成
        if not mission.is_repetitive and (await has_user_completed_mission(
            current_user, mission_id
        )):
        ...
        wait_for_review_mission = await MissionSubmitted.find_one(
            MissionSubmitted.mission_id == mission_id,
            MissionSubmitted.user_id == str(current_user.id),
            MissionSubmitted.session == app_conf.mission,
            MissionSubmitted.review_status == MissionReviewState.PENDDING,
...
async def mission_abandon_action(
    ...
    try:
        ...
        if not mission:
            request.session["error"] = "任務不存在"
            return RedirectResponse(
                url=request.url_for("user_index_page"), status_code=302
            )
...
async def mission_submit_action(
    ...
    try:
        ...
        # 檢查是否已經提交過審核
        existing_review = await MissionSubmitted.find_one(
            MissionSubmitted.mission_id == mission_id,
            MissionSubmitted.user_id == str(current_user.id),
            MissionSubmitted.session == app_conf.mission,
            MissionSubmitted.review_status == MissionReviewState.PENDDING,
        ...
        if existing_review:
            existing_review_mission = await Mission.get(
                PydanticObjectId(existing_review.mission_id)
            )
